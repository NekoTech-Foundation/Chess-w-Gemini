...
const handleSquareClick = (square: string) => {
  ...
  if (possibleDestinations.value.includes(square)) {
    emit('move', { from: selectedSquare.value, to: square });
    selectedSquare.value = null;
    possibleDestinations.value = [];
  } else {
    // Clicked elsewhere
...
<template>
  <div class="grid grid-cols-8 grid-rows-8 w-full max-w-[600px] aspect-square border-4 border-[#333] shadow-2xl rounded-sm overflow-hidden select-none">
    <!-- Render squares -->
