...
const { 
  ...
  makeMove, resetGame, loadGame: loadChessGame, getLegalMoves, getLegalMovesSAN,
...
const saveGame = () => {
    const gameState = {
        ...
        aiThought: aiThought.value,
        aiTaunt: aiTaunt.value,
        isGeminiDead: isGeminiDead.value
    };
...
const initGame = () => {
    try {
        ...
        if (saved) {
            ...
            // Resume if ID matches
            if (parsed.id === gameId && parsed.pgn) {
                const success = loadChessGame(parsed.pgn);
                if (success) {
                    lastMove.value = parsed.lastMove;
                    aiThought.value = parsed.aiThought;
                    aiTaunt.value = parsed.aiTaunt;
...
// Auto-save on every move (when FEN changes)
watch(fen, () => {
    saveGame();
});
...
const handleUserMove = async (move: { from: string; to: string }) => {
  ...
  if (result) {
    lastMove.value = move;
    saveGame(); // Save immediately after user move
    
    // Trigger AI if game not over
    if (!isGameOver.value) {
      await playAI();
      saveGame(); // Save after AI move
    }
  }
};
...
const playAI = async () => {
  ...
  const legalMoves = getLegalMovesSAN();
  ...
  <div class="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-4 md:p-8 font-sans">
    ...
    <header class="w-full max-w-6xl flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
      <div class="flex items-center gap-4">
        <button @click="goHome" class="p-2 rounded-full hover:bg-gray-800 text-gray-400 transition-colors">
        ...
        <span class="hidden md:flex px-3 py-1 rounded-full text-xs font-mono bg-gray-800 text-gray-400 border border-gray-700 items-center gap-2">
           {{ isGeminiDead ? 'State: Stockfish' : 'State: Gemini' }}
...
<style scoped>
/* Scoped styles tailored for the game view if needed, but App.vue styles were global */
/* Copying the animations and scrollbar from App.vue */
